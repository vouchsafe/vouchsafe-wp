...
function vouchsafe_render_settings_page() {
  ...
  <div class="wrap">
    ...
    <style>
      .vouchsafe-step{background:#fff;border:1px solid #dcdcde;border-radius:6px;padding:16px 20px;margin:16px 0;}
      .vouchsafe-step h2{margin:0 0 10px;font-size:1.2rem;}
      .vouchsafe-step .stepnum{display:inline-block;background:#2271b1;color:#fff;border-radius:999px;width:26px;height:26px;line-height:26px;text-align:center;margin-right:8px;font-weight:600;}
      .vouchsafe-actions{margin-top:10px;}
    ...
    <div class="vouchsafe-step">
      ...
      <p class="description">Paste your Client ID and Client Secret from the Vouchsafe dashboard.</p>
    ...
    <div class="vouchsafe-step">
      ...
      <p class="description">Fetch your published verification flows (via <code>GET /flows</code>) and select a default flow. This flow will be used unless a specific <code>workflow_id</code> is provided in the request URL.</p>

      <?php if (!$has_creds): ?>
        <div class="notice notice-warning"><p>Save your API credentials in Step 1 before fetching flows.</p></div>
      <?php endif; ?>
      ...
      <form method="post" action="options.php" style="margin-top:10px;">
        <?php
          ...
          if (is_array($flows) && !empty($flows)) {
            ...
            echo '<option value="">(Fetch flows first)</option>';
          }
          echo '</select>';
          if ($selected_flow) {
    ...
    <div class="vouchsafe-step">
      <h2><span class="stepnum">3</span> How to request verifications</h2>
      <ol>
        <li>Set your form’s confirmation redirect to: <code><?php echo esc_html( home_url('/vouchsafe/request') ); ?>?email={merge-tag}</code></li>
